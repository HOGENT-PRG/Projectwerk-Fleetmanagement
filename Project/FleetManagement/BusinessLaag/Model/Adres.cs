   ...
   public class Adres
    {
        ...
        {
            if (string.IsNullOrEmpty(straatnaam) || straatnaam.Any(char.IsDigit) || straatnaam.Length > 150) throw new AdresException("Straat moet ingevuld worden en mag geen cijfers bevatten, straat mag max 150 karakters lang zijn.");
            Straatnaam = straatnaam;
        }    
        public void zetHuisnummer(string huisnummer)
        {
            if (string.IsNullOrEmpty(huisnummer) || !(huisnummer.Any(char.IsDigit)) || huisnummer.Length > 50) throw new AdresException("Huisnummer moet ingevuld worden en ten minste 1 cijfer bevatten, max 50 karakters.");
        ...
        {
            if (string.IsNullOrEmpty(plaatsnaam) || plaatsnaam.Any(char.IsDigit) || plaatsnaam.Length > 150) throw new AdresException("Plaatsnaam moet ingevuld worden en mag geen cijfers bevatten, tevens heeft het een maximum van 150 karakters.");
            Plaatsnaam = plaatsnaam;
        } 
        public void zetProvincie(string provincie)
        {
            if (string.IsNullOrEmpty(provincie) || provincie.Any(char.IsDigit) || provincie.Length > 150) throw new AdresException("Provincie moet ingevuld worden en mag geen cijfers bevatten, mag maximum 150 karakters lang zijn.");
            Provincie = provincie;
        }
        public void zetLand(string land)
        {
            if (string.IsNullOrEmpty(land) || land.Any(char.IsDigit) || land.Length > 100) throw new AdresException("Land moet ingevuld worden en mag geen cijfers bevatten. Land mag maximaal bestaan uit 100 karakters.");
            Land = land;
        }
